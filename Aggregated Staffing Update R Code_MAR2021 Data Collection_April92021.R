...
df1<-read_excel("October 2020 Final Dataset/Staffing_Historical_Dataset_Mar42021.xlsx") %>%
...
#STEP 5: BIND TOGETHER ALL MARCH 2021 to OCT 2020
...
#ADD UNIQUE IDs FOR new rows that don't have them


##PART 2: STAFFLING LEVELS by OU x Healthfunding

#Group vacant positions dataset into a count of vacant positions for 
Staffing_Vacant_Oct2020<-read_excel("October 2020 Final Dataset/Staffing_Historical_Dataset_Mar42021.xlsx") %>%
  dplyr::filter(staffingstatus != "Filled") %>% 
  dplyr::filter(staffingstatus != "") %>% 
  dplyr::mutate(`Total Vacancies Oct2020`=1)  %>%
  dplyr::select('country', 'staffingstatus', 'healthfunding', 'Total Vacancies Oct2020') %>% 
  dplyr::group_by(country, `healthfunding`) %>% 
  dplyr::summarise(`Total Vacancies Oct2020` = sum(`Total Vacancies Oct2020`, na.rm = TRUE))
  

#Group vacant positions dataset into a count of vacant positions for MARCH
Staffing_Vacant_Mar2021<-Staffing_Aggregated_Final  %>% 
  dplyr::filter(staffingstatus != "Filled") %>% 
  dplyr::filter(`datacollection_date`!="October 2020") %>% 
  dplyr::filter(staffingstatus != "") %>% 
  dplyr::mutate(`Total Vacancies Mar2021`=1)  %>%
  dplyr::select('country', 'staffingstatus', 'healthfunding', 'Total Vacancies Mar2021') %>% 
  dplyr::group_by(country, `healthfunding`) %>% 
  dplyr::summarise(`Total Vacancies Mar2021` = sum(`Total Vacancies Mar2021`, na.rm = TRUE))
...
#Check to see staffling level groupings
Staffing_Levels_QC <- lapply(March_Data, read_xlsx_files_2 ) %>% 
  bind_rows() 
...
#binding vacancies 
 Staffing_Vacant_Totals <-right_join(Staffing_Vacant_Oct2020, Staffing_Vacant_Mar2021, "country" = "country", "healthfunding" = "healthfunding") %>%  
 dplyr::mutate(across(where(is.numeric), round, 0))
 
 #binding vacancies to staffing
 Staffing_Levels_Final <-left_join(Staffing_Levels_Total,Staffing_Vacant_Totals, "country" = "country", "healthfunding" = "healthfunding")  
...
Staffing_Levels_Final<- left_join(Staffing_Levels_Final, df.OU, by = "country") 
...
#Export Data  
  ...
  write_csv(  Staffing_Aggregated_Final,"Staffing_Master_Dataset_Apr9.csv")
  write_csv(  Staffing_Levels_Final,"StaffingCounts_Aggregated_Dataset_Apr9.csv")
