...
#Export Data  
  ...
  write_csv(Staffing_Aggregated_Final,"Staffing_Master_Dataset_May6_MissingUniqueIDs.csv")
  write_csv(Staffing_Levels_Final,"TotalStaffCounts_Aggregated_Dataset_May6.csv")

  
  #Export March comment only
  
  March_Comments <- df2 %>% 
    dplyr::filter(staffingstatus != "Filled") %>%
    dplyr::filter(hiringcascade != "12. Position Filled/Candidate started work") %>% 
    select ('country', 'staffingid', 'position', 'uniqueid', 'comments_Mar2021')
  
  write_csv(  March_Comments, "March_Comments_June3.csv")
