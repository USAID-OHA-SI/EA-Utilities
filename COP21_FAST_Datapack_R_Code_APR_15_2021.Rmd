...
df.DHI <-df.Earmarks_IM %>% 
      dplyr::select('Planning Cycle':'Digital Health Investments', 'Agency Category') %>% 
       dplyr::rename("Country" = `Operating Unit`) %>% 
      distinct(.keep_all= TRUE)
  
   df.DHI_Final <- merge(df.DHI, df.OU_Country, by = "Country")

  write_csv(df.DHI_Final,"COP21 FAST and Data Pack Submissions/Dataset/DHI_05_17_v1.csv")
```
