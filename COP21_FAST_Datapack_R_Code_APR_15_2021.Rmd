...
#tailor Datapack to append to FAST wrangling    
  ...
  dplyr::group_by(operatingunit, countryname, fundingagency, mech_code, primepartner, mech_name, indicator, fiscal_year) %>%
  ...
  df.COP21PlanningDataset <- df.COP21PlanningDataset %>%
    ...
    `Agency Category` = case_when(
     `Funding Agency` == "USAID/WCF"~ "USAID",
      ...
      dplyr::select('Planning Cycle':'Total Planned Funding','Data Stream', 'Agency Category', 'Cross-Cutting Attribution':'Digital Health Investments', 'Earmark') %>% 
   ...
   df.COP21PlanningDataset <- merge(df.COP21PlanningDataset, df.OU_Country, by = "Country")
    ...
    #RUN IF HAVE FASTS ONLY  
      #df.COP21PlanningDataset_2 <-df.COP21PlanningDataset %>% 
       #dplyr::rename("Country" = `Operating Unit`)
   
   #FINAL DATASET!
  #df.COP21PlanningDataset_Final <- merge(df.COP21PlanningDataset, df.OU_Country, by = "Country")
...
##Step8: Write the file to output folder.
Update the date
```{r}
  write_csv(df.COP21PlanningDataset_Final,"COP21 FAST and Data Pack Submissions/Dataset/FAST_DATAPACK_04_15_v1.csv")
```
