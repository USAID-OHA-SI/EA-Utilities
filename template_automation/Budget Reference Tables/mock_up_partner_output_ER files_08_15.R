...
fisc_dir = "ER23_Financial_Programmatic/"
...
gen_df_no_cost <- function(df, null_rm=TRUE){
  df <- 
    ...
    group_by(planning_cycle, fiscal_year, program, sub_program, interaction_type, 
             targeted_beneficiary, allocated_beneficiary) %>% 
...
template_path <- '~/GitHub/EA-Utilities/template_automation/Budget Reference Tables/partnerDataTemplateV1.xlsx'

dollar_cell <- createStyle(numFmt = "$#,##0")
...
wb_pipeline <- function(mech, dr = "", df = df_fsd){
  ...
  addStyle(wb, sheet = 2, style = dollar_cell, rows = 2:200, cols = 8, gridExpand=T, stack=T)
