...
header-includes: 
  ...
  - \allsectionsfont{\Large \raggedright \color{usaid_blue}} #fixed color of headers
...
get_ou_agency_be<- gen_budget_exec(df_budget_exec) %>%
  mutate(across(c(2:3,5:6), currency)) %>%
  mutate(across(c(4,7), percent)) 
...
get_ou_pa<- gen_budget_exec(df_budget_exec, "program")%>%
  mutate(across(c(3:4,6:7), currency)) %>%
  mutate(across(c(5,8), percent)) 
get_ou_pa %>%
...
df_ue <- gen_ue(fsd_tgt, msd_tgt)%>%
  mutate(across(c(3,5,7,9), currency))
...
local_table <- gen_local_table(df_local) %>%
  mutate(across(3:4, currency)) %>%
  mutate(across(5, function(x) x/100)) %>%
  mutate(across(5, percent)) %>%
...
hts_ue <- get_program_specific(df_ue_indiv, programs="HTS")%>%
  mutate(across(4:5, currency)) %>%
  mutate(across(6:8, function(x) x/100)) %>%
  mutate(across(6:8, percent))
...
hrh_main <- gen_hrh_main(df_hrh, df_fsd)%>%
  mutate(across(c(2,4,5), currency)) %>%
  mutate(across(6, function(x) x/100)) %>%
  mutate(across(6, percent))
...
hrh_staff <- gen_staff(df_hrh)%>%
  ungroup()%>%
  mutate(across(c(2:16)[c(T, F)], currency)) %>%
  mutate(across(c(3:17)[c(T, F)], function(x) x/100)) %>%
  mutate(across(c(3:17)[c(T, F)], percent))
...
hrh_mer <- gen_hrh_mer(df_hrh, msd_tgt) %>%
  ...
  mutate(across(c(2:10)[c(T, F)], currency)) %>%
...
mechXpa <- gen_budget_exec(df_fsd, group_col = c("mech", "program"))%>%
  mutate(across(c(4:5,6:8), currency)) %>%
