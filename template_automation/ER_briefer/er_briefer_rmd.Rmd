...
get_ou_agency_be<- gen_budget_exec(df_budget_exec) %>%
  mutate(across(c(2:3,5:6), currency,digits=0)) %>%
  mutate(across(c(4,7), percent,digits=0))
...
get_ou_pa<- gen_budget_exec(df_budget_exec, "program")%>%
  mutate(across(c(3:4,6:7), currency, digits=0)) %>%
  mutate(across(c(5,8), percent, digits=0))
...
df_ue <- gen_ue(fsd_tgt, msd_tgt)%>%
  mutate(across(c(3,5,7,9), currency,digits=0)) %>%
  mutate(across(c(4,6,8,10), comma, digits=0))

ue_cols <- c("", "Mechanism", "TST UE", "TST Results", "TST POS UE", "TEST POS Results",
...
local_table <- gen_local_table(df_local) %>%
  mutate(across(3:4, currency,digits=0)) %>%
  mutate(across(5, function(x) x/100)) %>%
  mutate(across(5, percent,digits=0)) %>%
...
PM spend proportion is another area of focus for all countries.  Program management is an important part of successful programs and it is important to note that the PM for international partners is largely driven by an organization’s NICRA, which is negotiated during contract negotiations.  While there may be ways to minimize other components of PM, reductions should not jeopardize having sufficient management and oversight in place. Below is a graph of the shift in spending by interaction type from FY18 onwards. 
...
Mechanisms focused on providing treatment services, or linkage to treatment, adherence, and retention services should also have expenditure within the C\&T program area. It is important to review whether that the mechanisms listed below have executed their C\&T budgets inline with their relevant programmatic (i.e. MER) performance. If a mechanism is overspending their budget with poor performance on the treatment indicators, be prepared to explain why that has occurred. One should review these specific mechanisms to ensure that the spending aligns with their programmatic purpose. Note\: Financial data is not disaggregated by specific indicators. 
```{r echo=FALSE, message=FALSE, warning=FALSE}
...
if(nrow(ovc_ue)!=0){
  ovc_ue <- ovc_ue %>%
    ...
    mutate(across(6:7, percent,digits=0))
...
The chart below provides an OU-level overview on HRH information (HRH FTE, HRH individuals, HRH expenditure, and total expenditure). Most OUs report anywhere from 30-60\% of the total OU expenditure is for HRH, and HRH is one of PEPFAR’s top cost drivers. 

```{r echo=FALSE, message=FALSE, warning=FALSE}
hrh_main <- gen_hrh_main(df_hrh, df_fsd)%>%
  mutate(across(c(2,4,5), currency,digits=0)) %>%
  mutate(across(6, function(x) x/100)) %>%
  mutate(across(6, percent,digits=0)) %>%
...
In the chart below, we can see the breakdown of expenditure on HRH by ER category (Ancillary, Clinical, Other Staff, and Program Management (PM)) in `r params$ou`. Note: All Ancillary and Clinical Staff are Service Delivery and all Other and PM Staff are Non Service Delivery. Depending on the programmatic goals of a mechanism, it will be necessary to understand how this aligns with the staff breakdown. This information is presented at the agency level. 
```{r echo=FALSE, message=FALSE, warning=FALSE}
hrh_staff <- gen_staff(df_hrh)%>%
  ungroup()%>%
  mutate(across(c(2:16)[c(T, F)], currency,digits=0)) %>%
  mutate(across(c(3:17)[c(T, F)], function(x) x/100)) %>%
  mutate(across(c(3:17)[c(T, F)], percent,digits=0)) %>%
  arrange(factor(fundingagency, levels= agency_lvls), desc(fundingagency))

hrh_cols <- c("Agency", "Actual Annual Spend", "Spend Share", "Annual FTE", "FTE Share")

hrh_ancil <- hrh_staff %>%select(c(1, 2:5))
hrh_clinic <- hrh_staff %>% select(c(1, 6:9))
...
```
## Comparing Tx_CURR to  Care and Treatment (C\&T) HRH FTE and Expenditures
Given the importance of treatment programs to sustaining PEPFAR’s success, the chart below presents how C\&T-SD spending and FTE align with TX_CURR achievement.  While a direct link between the HRH and MER data is difficult to discern, it is important to reflect on how the balance of C\&T FTE aligns with treatment-related indicators. For example, a funding agency spending large amounts on C\&T with low achievement may require greater programmatic support. There is no ideal expenditure or total FTE that country teams should aim for, however you can make comparisons across agencies and mechanisms to better understand HRH investments that support Tx_CURR target achievement.

```{r echo=FALSE, message=FALSE, warning=FALSE}
hrh_mer <- gen_hrh_mer(df_hrh, msd_tgt) %>%
  ungroup() %>%
  # select every other # between 2 and 10, and between 3 and 11
  mutate(across(c(2:10)[c(T, F)], currency,digits=0)) %>%
  mutate(across(c(3:11)[c(T, F)], function(x) x/100)) %>%
  mutate(across(c(3:11)[c(T, F)], percent,digits=0)) %>%
  arrange(factor(fundingagency, levels= agency_lvls), desc(fundingagency))
...
hrh_mer %>%
  ...
  add_header_above(c(" "=1, "SD C&T"=3, "NSD C&T"=3, "Totals"=2)) %>%
...
mechXpa <- gen_budget_exec(df_fsd, group_col = c("mech", "program"))%>%
  # Remove mechs with missing FY21 ER or budget data
  dplyr::filter(expenditure_amt_2021>0 | cop_budget_total_2021>0) %>%
  mutate(across(c(4:5,6:8), currency,digits=0)) %>%
  mutate(across(c(6,9), percent, digits=0))
